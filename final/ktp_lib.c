...
//     if (sockets[ktp_sockfd_id].swnd.swndWindow == 0)
//     {
//         sem_post(semaphore); // Unlock semaphore
//         return ENOSPACE;
//     }
...
//     struct message m;
//     strcpy(m.msg, mess);
//     if(strlen(m.msg)<maxSize) m.msg[strlen(m.msg) - 1] = '\0';
//     else m.msg[strlen(m.msg) - 1] ='\0';
...
//     sockets[ktp_sockfd_id].swnd.messageBuffer[sockets[ktp_sockfd_id].swnd.end] = m;
//     sockets[ktp_sockfd_id].swnd.end = (sockets[ktp_sockfd_id].swnd.end + 1) % WINDOW_SIZE;
//     sockets[ktp_sockfd_id].swnd.swndWindow--;
//     printf("[SEND] Attempting to send %s message with seq %d \n\n", sockets[ktp_sockfd_id].swnd.messageBuffer[sockets[ktp_sockfd_id].swnd.end - 1].msg, sockets[ktp_sockfd_id].swnd.messageBuffer[sockets[ktp_sockfd_id].swnd.end - 1].seq);
//     sem_post(semaphore); // Unlock semaphore
//     sleep(5);
//     return 0;
// }
...
//     sem_wait(semaphore); // Lock semaphore
//     struct KTP_socket *sockets = get_shared_resources();
//     // printf("Socket %d isFree: %s, Port: %d, UDP FD: %d and Send Message bufer size: %d\n",
//     //        ktp_sockfd, KTP_sockets[ktp_sockfd].isFree ? "true" : "false",
//     //        !KTP_sockets[ktp_sockfd].isFree ? KTP_sockets[ktp_sockfd].port : -1,
//     //        KTP_sockets[ktp_sockfd].udpSocketId,
//     //        KTP_sockets[ktp_sockfd].swnd.seq_number);
...
//     while (sockets[ktp_sockfd].rwnd.base == sockets[ktp_sockfd].rwnd.end || sockets[ktp_sockfd].rwnd.rcvd[sockets[ktp_sockfd].rwnd.base]==false)
//     {
//         sem_post(semaphore); // Unlock semaphore temporarily
//         printf("No data so waiting ....\n");
//         sleep(10);           // Wait for data
//         sem_wait(semaphore); // Lock semaphore again
//     }
//     strcpy(message, sockets[ktp_sockfd].rwnd.messageBuffer[sockets[ktp_sockfd].rwnd.base]);
